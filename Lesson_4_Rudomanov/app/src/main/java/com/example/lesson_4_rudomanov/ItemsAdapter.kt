...
class ItemsAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ...
            TYPE_DETAIL_BASE -> DetailLikeBaseHolder(parent, detailListener)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            ...
            is BaseViewHolder -> holder.bind((items[position] as ListItem.BaseInfoItemSeal).base)
            is DetailLikeBaseHolder -> holder.bind((items[position] as ListItem.DetailInfoItemSeal).detail)
        }
    }

    override fun getItemViewType(position: Int): Int {
        return if (items[position] is ListItem.DetailInfoItemSeal && position + 1 < items.size && items[position + 1] is ListItem.BaseInfoItemSeal) {
            TYPE_DETAIL_BASE
